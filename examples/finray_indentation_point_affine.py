...
OUT_DIR = init_robot_demo(args, "finray_indentation_pointy2", "finray_indentation_pointy2")
...
if __name__ == "__main__":
    ...
    initial_pointy_pos = np.array([0.19, 0.02, 0.0])

    #0.33-pointy3  0.19 - pointy2  0.25-pointy1
    left_softfinger_handles, objects_pointy_handles = [], []
    for env_id in range(args.num_envs):
        mesh1 = left_softfinger.copy()
        mesh3 = objects_pointy.copy()

        mesh3.points = mesh3.points @ Rx(np.radians(-90)) @ Ry(np.radians(-90)) @ Rz(np.radians(90)) + env_pos[env_id] + initial_pointy_pos
    ...
    for t in tqdm(range(total_timesteps)):
        for env_id in range(args.num_envs):
            ...
            if t < int(total_timesteps/2):
                # SLIDING UP - move pointy object up to indent finger
                ...
                # LIFTING DOWN - move pointy object back down
            ...
            # Identify nodes in contact (non-zero contact force)
            ...
            # Threshold to determine if a node is in contact (adjust as needed)
            contact_threshold = 1e-6
            nodes_in_contact = torch.where(total_force_magnitudes > contact_threshold)[0]

            # Get the positions of nodes in contact
            ...
            if len(nodes_in_contact) > 0:
                ...
                # Store contact positions and forces for JSON
                ...
                print(f"[t={t:03d}] Nodes in contact: {len(nodes_in_contact)}")
                print(f"  Contact node indices: {nodes_in_contact.cpu().numpy()[:10]}...")  # First 10
                print(f"  Max contact force: {contact_force_magnitudes.max().item():.4f}")
                print(f"  Total contact force: {contact_force_magnitudes.sum().item():.4f}")
                
                # Print first few contact positions
                if len(contact_node_positions) > 0:
                    print(f"  First contact point: {contact_node_positions[0]['position']}")
        ...
        # Prepare data to save (OUTSIDE viz block, correct indentation)
        record_dict = {
            "timestamp": t * dt,
            "timestep": t,
            ...
            "contact_info": {
                ...
                "contact_points": contact_node_positions  # ADD THIS - includes positions and forces
            },
            "net_forces": {
            ...
            "tracked_nodes": {
                ...
                "positions": [left_targets[idx].cpu().numpy().tolist() if idx < len(left_targets) else [0,0,0] for idx in tracked_indices],
                "deformations": [(left_targets[idx] - rest_left_positions[idx]).cpu().numpy().tolist() if idx < len(left_targets) else [0,0,0] for idx in tracked_indices]
            }
        }

        # Write to JSON file with formatted output
        with open(json_path, 'a') as f:
            # Format JSON with custom line breaks for readability
            ...
            # Write the formatted JSON (no need to check for t==0 anymore)
            f.write(json_str)
        ...
        if t % 10 == 0:
            print(f"Saved data for timestep {t} to {json_path}")
