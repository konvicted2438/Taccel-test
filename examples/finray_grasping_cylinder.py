...
OUT_DIR = init_robot_demo(args, "finray_grasping", "finray_grasping_cylinder")
...
if __name__ == "__main__":
    ...
    objects_sphere = pv.read("finray_test/cylinder_side_vol.msh")

    scale = 0.005  # uniform scale factor

    left_softfinger.points *= scale
    objects_sphere.points *= 0.012
    ...
    for env_id in range(args.num_envs):
        ...
        mesh1.points = mesh1.points @ Rx(np.radians(-90)) @ Ry(np.radians(90)) @ Rz(np.radians(-90)) + env_pos[env_id] + np.array([0.2, 0.5, 0.0]) #(106,3) @ (3,3) + (3,)
        ...
        #  # Rx/y/z(theta) rotates theta degrees CCW when looking from origin toward +x/y/z
        # # NOTE: From left to right, Rx acts first, then Ry - and the matrix multiplication is NOT commutative!
        mesh2.points = mesh2.points @ Rx(np.radians(-90)) @ Ry(np.radians(-90)) @ Rz(np.radians(90)) + env_pos[env_id] + np.array([0.5, 0.5, 0.0]) #(106,3) @ (3,3) + (3,)
        ...
        mesh3.points = mesh3.points @ Rx(np.radians(90)) @ Ry(np.radians(-90)) + env_pos[env_id] + np.array([0.35, 0.01, 0.0])
