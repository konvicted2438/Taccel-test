...
def create_soft_gripper_benchmarkTO(nelx, nely, nelz, volfrac, penal, rmin, ft, threshold, OUT_DIR, max_loops = 50):
    ...
    xPhys = x.copy() #I'm using xPhys rather than xPhys in the paper. shape = (N, )
    loop = 0
    change = 1
    ...
    while change > 0.01 and loop < max_loops:
        loop += 1
        sK = (KE.reshape((KE.shape[0]*KE.shape[1], 1), order = "C") * (Emin + xPhys.reshape(1, -1) ** penal * (E0 - Emin))).flatten(order = "F") # shape = (N*8*8, )
        ...
        u_dash = (penal * xPhys ** (penal - 1) * (E0 - Emin)) * ((U_out[edofMat].reshape(nelx * nely, 8) @ KE) * U_in[edofMat].reshape(nelx * nely, 8)).sum(axis=1)
        v_dash = (penal * xPhys ** (penal - 1) * (E0 - Emin)) * ((U_out[edofMat].reshape(nelx * nely, 8) @ KE) * U_out[edofMat].reshape(nelx * nely, 8)).sum(axis=1)
        dc = (u_dash*v - u*v_dash)/(v**2)
        dv = np.ones(nely * nelx) #shape = (N,)

        # Filtering/modification of sensitivities
        if ft == 1:
            dc = np.asarray((H * (xPhys * dc))[np.newaxis].T / Hs)[:, 0] / np.maximum(0.001, x) 
        elif ft == 2:
        ...
        while (l2 - l1)/(l2 + l1) > 1e-4 and l2 > 1e-40:
            ...
            if ft == 1:
                ...
                xPhys = np.asarray(H * xnew[np.newaxis].T / Hs)[:, 0] #new: density filtering | shape = (N, )
...
class TopologyOptimizer2D(object):
    ...
    def SDTO_2D(self, volfrac, penal, rmin, ft, threshold, max_loops = 50, angle = 45, E = 1, xsolv = "OC"):

        assert xsolv in ["MMA", "OC"], "You must specify either an MMA or OC optimizer."
        ...
        # =========== INITIALISE MMA INPUTS
        if xsolv == "MMA":
            m = 1 
            n = self.nely*self.nelx
            xval = x.reshape(n,1, order = "F") # (N,1)
            xmin = np.zeros((n,1)) # (N,1)
            xmax = np.ones((n,1))  # (N,1)
            xold1 = xval.copy()    # (N,1)
            xold2 = xval.copy()    # (N,1)
            low = np.zeros((n,1))  # (N,1)
            upp = np.ones((n,1))   # (N,1)
            mma_a0 = 1.0 
            mma_a = np.zeros((m,1)) 
            mma_c = 1000*np.ones((m,1))
            mma_d = np.zeros((m,1))
            mma_move = 0.2 
        ...
        while change > 0.01 and loop < max_loops:
            ...
            # =========== OBJECTIVE FUNCTION AND SENSITIVITY ANALYSIS
            c, dc = self.get_c_and_dc(U, xPhys, penal, K, E0, Emin, KE, edofMat)
            ...
            # =========== OC OPTIMIZER: Update mathematical density, x, and set new xPhys (for FEA) to xnew
            if xsolv == "OC":
                l1, l2, move = 0, 100000, 0.1
                while (l2 - l1)/(l2 + l1) > 1e-4 and l2 > 1e-40:
                    lmid = 0.5 * (l2 + l1)
                    ...
                    #xnew.reshape(nely,nelx, order = 'F')[passive==2] = 1
                    if np.sum(xPhys) > volfrac * self.nelx * self.nely:
                        l1 = lmid
                    else:
                        l2 = lmid
            ...
            # =========== MMA UPDATE OF DESIGN VARIABLES AND PHYSICAL DENSITIES
                ...
                #xval = x.copy().reshape(nelx*nely, 1, order = "C")
                xmma,ymma,zmma,lam,xsi,eta,mu,zet,s,low,upp = \
                    mmasub(m,n,loop,xval,xmin,xmax,xold1,xold2,f0val,df0dx,fval,dfdx,low,upp,mma_a0,mma_a,mma_c,mma_d,mma_move)
                xold2 = xold1.copy()
                xold1 = xval.copy()
                xval = xmma.copy()
                # =========== FILTER x
                if ft == 1: 
                    xPhys = xval.reshape(self.nely*self.nelx, order = "F")
                elif ft == 2: 
                    xPhys = np.asarray(H * xval / Hs)[:, 0] #new: density filtering | shape = (N, )
                xval.reshape(self.nely,self.nelx, order = 'F')[passive==1] = 0
                #xval.reshape(nely,nelx, order = 'F')[passive==2] = 1
                change = np.max(np.abs(x.reshape(self.nelx*self.nely,order="F")-xval.reshape(self.nelx*self.nely,order="F")))
                x = xval.reshape(self.nelx*self.nely, order = 'F')
    ...
    """
    def get_c_and_dc(self, U, xPhys, penal, K, E0, Emin, KE, edofMat):
        ...
        for i in np.arange(len(self.cps2maxforces_2D)) + 1:
            ...
            MSE = (U_out.T @ K @ U_in).item() # These are correct as they include PERTURBED stiffness matrices K
            SE = (U_out.T @ K @ U_out).item()  # These are correct as they include PERTURBED stiffness matrices K
            c += force_norms[i-1] * MSE/(SE)

            u = (U_out.T @ K @ U_in).item() 
            v = (U_out.T @ K @ U_out).item()
            u_dash = (penal * xPhys ** (penal - 1) * (E0 - Emin)) * ((U_out[edofMat].reshape(self.nelx * self.nely, 8) @ KE) * U_in[edofMat].reshape(self.nelx * self.nely, 8)).sum(axis=1)
            v_dash = (penal * xPhys ** (penal - 1) * (E0 - Emin)) * ((U_out[edofMat].reshape(self.nelx * self.nely, 8) @ KE) * U_out[edofMat].reshape(self.nelx * self.nely, 8)).sum(axis=1)
...
def main():
    ...
    # ft == 2 --> density filter
    binary_3D_design = topology_optimizer.SDTO_2D(volfrac = args.TO_volfrac, penal = 3, rmin = 7, ft = 2, threshold = args.TO_threshold, max_loops = args.TO_max_loops,
                                                  angle = 45, E = 1, xsolv = "MMA")
